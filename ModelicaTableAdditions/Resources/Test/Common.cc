...
char* ModelicaAllocateString(size_t len) {
    auto *data = malloc(len + 1); /* Never free'd in the test programs */
    if (NULL == data) {
...
char* ModelicaAllocateStringWithErrorReturn(size_t len) {
    return static_cast<char*>(malloc(len + 1)); /* Never free'd in the test programs */
}

char* ModelicaDuplicateString(const char* str) {
    auto *data = static_cast<char*>(malloc(strlen(str) + 1)); /* Never free'd in the test programs */
    if (NULL == data) {
