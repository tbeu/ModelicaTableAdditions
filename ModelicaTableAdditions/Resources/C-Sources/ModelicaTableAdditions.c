/* ModelicaTableAdditions.c - External table functions
...
/* Implementation of external functions for table computation
   in the ModelicaTableAdditions Library:

      ModelicaTableAdditions.Blocks.Sources.CombiTimeTable
      ModelicaTableAdditions.Blocks.Tables.CombiTable1Ds
      ModelicaTableAdditions.Blocks.Tables.CombiTable1Dv
      ModelicaTableAdditions.Blocks.Tables.CombiTable2Ds
...
typedef struct CombiTimeTable {
    ...
    double preNextTimeEventCalled; /* Time of previous call of
        ModelicaTableAdditions_CombiTimeTable_nextTimeEvent, discrete */
...
#pragma G2_DEFINE_CONSTRUCTOR_PRAGMA_ARGS(G2_FUNCNAME(ModelicaTableAdditions_initializeCS))
#endif
G2_DEFINE_CONSTRUCTOR(G2_FUNCNAME(ModelicaTableAdditions_initializeCS))
static void G2_FUNCNAME(ModelicaTableAdditions_initializeCS)(void) {
    InitializeCriticalSection(&cs);
}
#ifdef G2_DEFINE_DESTRUCTOR_NEEDS_PRAGMA
#pragma G2_DEFINE_DESTRUCTOR_PRAGMA_ARGS(G2_FUNCNAME(ModelicaTableAdditions_deleteCS))
#endif
G2_DEFINE_DESTRUCTOR(G2_FUNCNAME(ModelicaTableAdditions_deleteCS))
static void G2_FUNCNAME(ModelicaTableAdditions_deleteCS)(void) {
...
void* ModelicaTableAdditions_CombiTimeTable_init(_In_z_ const char* tableName,
                                                 ...
                                                 int extrapolation) {
    return ModelicaTableAdditions_CombiTimeTable_init2(fileName,
        tableName, table, nRow, nColumn, startTime, columns, nCols, smoothness,
        extrapolation, startTime, ALWAYS, 1 /* verbose */);
}

void* ModelicaTableAdditions_CombiTimeTable_init2(_In_z_ const char* fileName,
...
void* ModelicaTableAdditions_CombiTimeTable_init3(_In_z_ const char* fileName,
    ...
    if (NULL == tableID) {
        ...
        if (NULL != file) {
            MUTEX_LOCK();
            if (--file->refCount == 0) {
                ModelicaIOAdditions_freeRealTable(file->table);
                free(file->key);
...
double ModelicaTableAdditions_CombiTimeTable_getValue(void* _tableID, int iCol,
...
double ModelicaTableAdditions_CombiTimeTable_getDerValue(void* _tableID, int iCol,
                                                         double t,
...
double ModelicaTableAdditions_CombiTimeTable_getDer2Value(void* _tableID, int iCol,
                                                         double t,
                                                         double nextTimeEvent,
                                                         double preNextTimeEvent,
                                                         double der_t,
...
double ModelicaTableAdditions_CombiTimeTable_nextTimeEvent(void* _tableID,
                                                           double t) {
...
double ModelicaTableAdditions_CombiTimeTable_read(void* _tableID, int force,
                                                  int verbose) {
#if !defined(NO_FILE_SYSTEM)
    CombiTimeTable* tableID = (CombiTimeTable*)_tableID;
    assert(NULL != tableID);
...
void* ModelicaTableAdditions_CombiTable1D_init(_In_z_ const char* tableName,
                                               _In_z_ const char* fileName,
                                               _In_ const double* table, size_t nRow,
                                               size_t nColumn,
                                               _In_ const int* columns,
                                               size_t nCols, int smoothness) {
    return ModelicaTableAdditions_CombiTable1D_init2(fileName, tableName,
        table, nRow, nColumn, columns, nCols, smoothness, LAST_TWO_POINTS,
        1 /* verbose */);
}

void* ModelicaTableAdditions_CombiTable1D_init2(_In_z_ const char* fileName,
                                                _In_z_ const char* tableName,
                                                _In_ const double* table, size_t nRow,
                                                size_t nColumn,
                                                _In_ const int* columns,
...
double ModelicaTableAdditions_CombiTable1D_getValue(void* _tableID, int iCol,
                                                    double u) {
    double y = 0.;
    CombiTable1D* tableID = (CombiTable1D*)_tableID;
    assert(NULL != tableID);
...
double ModelicaTableAdditions_CombiTable1D_getDerValue(void* _tableID, int iCol,
...
double ModelicaTableAdditions_CombiTable1D_getDer2Value(void* _tableID, int iCol,
                                                       double u, double der_u,
                                                       double der2_u) {
    double der2_y = 0.;
    CombiTable1D* tableID = (CombiTable1D*)_tableID;
...
double ModelicaTableAdditions_CombiTable1D_read(void* _tableID, int force,
                                                int verbose) {
#if !defined(NO_FILE_SYSTEM)
    CombiTable1D* tableID = (CombiTable1D*)_tableID;
    assert(NULL != tableID);
...
void* ModelicaTableAdditions_CombiTable2D_init(_In_z_ const char* tableName,
                                               _In_z_ const char* fileName,
                                               _In_ const double* table, size_t nRow,
                                               size_t nColumn, int smoothness) {
    return ModelicaTableAdditions_CombiTable2D_init2(fileName, tableName,
        table, nRow, nColumn, smoothness, LAST_TWO_POINTS, 1 /* verbose */);
}

void* ModelicaTableAdditions_CombiTable2D_init2(_In_z_ const char* fileName,
                                                _In_z_ const char* tableName,
                                                _In_ const double* table, size_t nRow,
                                                size_t nColumn, int smoothness,
                                                int extrapolation,
...
double ModelicaTableAdditions_CombiTable2D_getValue(void* _tableID, double u1,
                                                    double u2) {
    double y = 0;
    CombiTable2D* tableID = (CombiTable2D*)_tableID;
    assert(NULL != tableID);
...
double ModelicaTableAdditions_CombiTable2D_getDerValue(void* _tableID, double u1,
...
double ModelicaTableAdditions_CombiTable2D_getDer2Value(void* _tableID, double u1,
                                                       double u2, double der_u1,
...
double ModelicaTableAdditions_CombiTable2D_read(void* _tableID, int force,
                                                int verbose) {
#if !defined(NO_FILE_SYSTEM)
    CombiTable2D* tableID = (CombiTable2D*)_tableID;
    assert(NULL != tableID);
...
static READ_RESULT readTable(_In_z_ const char* fileName, _In_z_ const char* tableName,
    ...
    if (NULL != tableName && NULL != fileName && NULL != nRow && NULL != nCol) {
        ...
        if (NULL != key) {
            ...
            if (NULL == file || force) {
                /* Release resources since ModelicaIOAdditions_readRealTable may fail with
                   ModelicaError
                */
                MUTEX_UNLOCK();
                free(key);
                ...
                if (NULL == key) {
                    ModelicaIOAdditions_freeRealTable(table);
                    return file;
                }
                strcpy(key, fileName);
                strcpy(key + lenFileName + 1, tableName);
